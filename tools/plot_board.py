...
class PlotBoard:
    ...
    def plot_atari_state(feature, c, h, w):
    ...
    def plot_single_atari_state(outputs, row, r, n, c, h, w, subtext_list):
        for i in range(len(outputs)):
            ax1 = plt.subplot(row, n, (r * n) + i + 1)
            image_r = outputs[i][c - 3].cpu().detach().numpy().clip(0, 1).reshape(h, w)
            image_g = outputs[i][c - 2].cpu().detach().numpy().clip(0, 1).reshape(h, w)
            image_b = outputs[i][c - 1].cpu().detach().numpy().clip(0, 1).reshape(h, w)
            plt.imshow((np.dstack((image_r, image_g, image_b)) * 255.999).astype(np.uint8))
            ax1.get_xaxis().set_visible(False)
            ax1.get_yaxis().set_visible(False)
            plt.text(0, -1, subtext_list[i], fontsize=60, color='black')
    ...
    def plot_go_board(boardsize, blackboard, whiteboard, cardinality=None, addtext=None):
        ...
        for x in range(boardsize):
            for y in range(boardsize):
                ...
                blackpiecevalue, whitepiecevalue = PlotBoard.adjust_piece_color_ratio(blackpiecevalue, whitepiecevalue)
                if (blackpiecevalue > whitepiecevalue):
                    ax.add_patch(plt.Circle((x, y), 0.35, fill=True, color='black', alpha=blackpiecevalue))
                    ax.add_patch(plt.Circle((x, y), 0.35, fill=True, color='white', alpha=whitepiecevalue))
                else:
                    ax.add_patch(plt.Circle((x, y), 0.35, fill=True, color='white', alpha=whitepiecevalue))
                    ax.add_patch(plt.Circle((x, y), 0.35, fill=True, color='black', alpha=blackpiecevalue))
        ...
        if (out_file is not None):
            try:
                ...
                if e.errno == errno.ENAMETOOLONG:
                    ...
                    short = name[:20]
                    idx = 1
                    while os.path.exists(os.path.join(out_dir, f"{short}~{idx}{ext}")):
                        idx += 1
                    out_file = f"{short}~{idx}{ext}"
                    print(f"Warning: File name too long, saved instead as '{out_file}'.")
                    PlotBoard.savefig(out_dir, out_file, game)
                    return
                else:
                    raise
        else:
    ...
    def merge_images_vertical(image_path1, image_path2, output_path):
        ...
        merged_image = Image.new('RGB', (width, height1 + height2))
    ...
    def merge_images_vertical(image_paths, output_path):
